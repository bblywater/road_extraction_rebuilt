"""
DLGU-Net V2：在原版 DLGU-Net 基础上改进 DLAM 模块
改进点：DLAM 加入可学习缩放因子（LayerScale），初始化为 0，
让注意力从恒等映射开始逐渐生效，避免训练初期特征分布被扰乱。
"""
...
class DLAM_V2(nn.Module):
    """改进版 DLAM：加入可学习缩放因子，初始化为 0"""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x + self.scale * x * combined


class DLGUNetV2(nn.Module):
    """DLGU-Net V2：使用改进版 DLAM_V2 的 U-Net"""
    def __init__(
        self,
        in_channels: int = 3,
        out_channels: int = 1,
        ...
        self.out = OutputLayer(init_channels, out_channels, apply_sigmoid=apply_sigmoid)
        self.dlam1 = DLAM_V2(init_channels, reduction=dlam_reduction, spatial_kernel=dlam_spatial_kernel)
        self.dlam2 = DLAM_V2(init_channels * 2, reduction=dlam_reduction, spatial_kernel=dlam_spatial_kernel)
        self.dlam3 = DLAM_V2(init_channels * 4, reduction=dlam_reduction, spatial_kernel=dlam_spatial_kernel)
        self.dlam4 = DLAM_V2(init_channels * 8, reduction=dlam_reduction, spatial_kernel=dlam_spatial_kernel)
